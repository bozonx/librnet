...
export class WsServerInstance extends DriverInstanceBase<WsServerProps> {
  ...
  async destroy() {
    ...
    if (!this.isInitialized()) {
      return this.ctx.log.error(
        `WsServerLogic.destroy: Server hasn't been initialized yet.`
      );
    }
   ...
   */
  onConnection(
    cb: (connectionId: string, request: WsServerConnectionParams) => void
  ): number {
  ...
  */
  ...
  handleConnectionClose(
    ...
    this.ctx.log.debug(
      `WsServerLogic connection ${connectionId} has been closed on server ${
        this.props.host
      }:${this.props.port} has been closed. Code ${code}. Reason: ${
        reason || ''
      }`
    );
  ...
  handleConnectionUnexpectedResponse(
    connectionId: string,
