    ...
    return new Promise((resolve, reject) => {
      exec(
        ...
        (error: ExecException | null, stdout: string, stderr: string) => {
          ...
          const ramTotalMatchRes = stdout.match(/Mem:\s+\d+\s+(\d+)/);
    ...
    return new Promise((resolve, reject) => {
      exec(
        ...
        (error: ExecException | null, stdout: string, stderr: string) => {
          ...
          const archDoc: Record<string, string> | undefined = data.lscpu.find(
            (el: Record<string, string>) => osArchKeys.includes(el.data)
          );
          const cpuDoc: Record<string, string> | undefined = data.lscpu.find(
            (el: Record<string, string>) => el.field === 'CPU(s):'
          );
