...
export class ServicesManager extends EntityManagerBase {
  ...
  async init() {
    for (const serviceName of Object.keys(this.services)) {
      ...
      if (service.init) {
        this.ctx.log.debug(
          `ServicesManager: initializing service "${serviceName}"`
        );
        // TODO: добавить таймаут инициализации
        this.changeStatus(
          serviceName,
          SERVICE_STATUS.initializing as ServiceStatus
        );
        ...
        try {
          ...
          this.ctx.log.error(
            `ServicesManager: service "${serviceName}" init error: ${e}`
          );
          this.changeStatus(
            serviceName,
            SERVICE_STATUS.initError as ServiceStatus
          );
        ...
        this.changeStatus(
          serviceName,
          SERVICE_STATUS.initialized as ServiceStatus
        );
      }
    }
  }
  ...
  async destroy() {
    for (const serviceName of Object.keys(this.services)) {
      ...
      if (service.destroy) {
        this.ctx.log.debug(
          `ServicesManager: destroying service "${serviceName}"`
        );
        this.changeStatus(
          serviceName,
          SERVICE_STATUS.destroying as ServiceStatus
        );
        ...
        try {
          await service.destroy(
            SERVICE_DESTROY_REASON.systemDestroying as ServiceDestroyReason
          );
        } catch (e) {
          this.ctx.log.error(
            `Service "${serviceName} destroying with error: ${e}"`
          );
        ...
        this.changeStatus(
          serviceName,
          SERVICE_STATUS.destroyed as ServiceStatus
        );
      }
    }
  }
  ...
  async startService(serviceName: string) {
    ...
    if (service.props.required) {
      const runningRequired: string[] = service.props.required.filter(
        (srvName: string) => {
          if (
            this.statuses[srvName] === (SERVICE_STATUS.running as ServiceStatus)
          )
            return true;
        }
      );
      ...
      if (runningRequired.length === service.props.required.length) {
        ...
        const handlerIndex = this.ctx.events.addListener(evenName, () => {
          const runningRequired: string[] = service.props.required.filter(
            (srvName: string) => {
              if (
                this.statuses[srvName] ===
                (SERVICE_STATUS.running as ServiceStatus)
              )
                return true;
            }
          );
          ...
          if (runningRequired.length === service.props.required.length) {
            ...
            this.startServiceRightNow(serviceName).catch((e) =>
  ...
  async stopService(serviceName: string, force?: boolean) {
    ...
    try {
      ...
      this.ctx.log.error(
        `ServicesManager: service "${serviceName}" stop error: ${e}`
      );
  ...
  private async startServiceRightNow(serviceName: string) {
    ...
    try {
      ...
      this.ctx.log.error(
        `ServicesManager: service "${serviceName}" start error: ${e}`
      );
      this.changeStatus(
        serviceName,
        SERVICE_STATUS.startError as ServiceStatus
      );
  ...
  private handleServiceFall(err: SubprogramError, serviceName: string) {
    this.ctx.log.error(
      `Service "${serviceName}" has gone to failed state: ${JSON.stringify(
        err
      )}`
    );
